...
public class BODamageUtils {
    ...
    // private static ExplosionImpl explosion;
    ...
    private static void onGameJoined(GameJoinedEvent event) {
        // explosion = new ExplosionImpl((ServerWorld) mc.world, herobrine, yourMom,
        // skibidi, Vec3d.ZERO, 6.0F, false,
        // Explosion.DestructionType.DESTROY);
        raycastContext = new RaycastContext(null, null, RaycastContext.ShapeType.COLLIDER,
                RaycastContext.FluidHandling.ANY, mc.player);
        bedRaycast = new RaycastContext(null, null, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.ANY,
                mc.player);
    }

    // Crystal damage
    public static double crystal(PlayerEntity player, Box bb, Vec3d crystal, BlockPos ignore, boolean ignoreTerrain) {
        if (SettingUtils.oldDamage())
            return oldVerCrystal(player, bb, crystal, ignore, ignoreTerrain);
        return crystalDamage(player, bb, crystal, ignore, ignoreTerrain);
    }

    public static double crystalDamage(PlayerEntity player, Box bb, Vec3d crystal, BlockPos obsidianPos,
        ...
        double damage = DamageUtils.crystalDamage(player, crystal, true, obsidianPos);

        return damage < 0 ? 0 : damage;
        // ((IVec3d) vec3d).meteor$set((bb.minX + bb.maxX) / 2, bb.minY, (bb.minZ +
        // bb.maxZ) / 2);
        ...
        // double exposure = getExposure(crystal, player, bb, raycastContext,
        // obsidianPos, ignoreTerrain);
        // double impact = (1 - (modDistance / 12)) * exposure;
        // double damage = ((impact * impact + impact) / 2 * 7 * (6 * 2) + 1);

        // damage = getDamageForDifficulty(damage);
        // damage = DamageUtil.getDamageLeft(mc.player, (float) damage,
        // mc.world.getDamageSources().explosion(player, player), (float)
        // player.getArmor(),
        // (float)
        // player.getAttributeInstance(EntityAttributes.ARMOR_TOUGHNESS).getValue());
        // damage = resistanceReduction(player, damage);

        // ((IExplosion) explosion).meteor$set(crystal, 6, false);
        // damage = blastProtReduction(player, damage, explosion);

        // return damage < 0 ? 0 : damage;
    }

    public static float oldVerCrystal(PlayerEntity player, Box bb, Vec3d crystal, BlockPos ignore,
            boolean ignoreTerrain) {
        ((IVec3d) vec3d).meteor$set((bb.minX + bb.maxX) / 2, bb.minY, (bb.minZ + bb.maxZ) / 2);
        ...
        damage = getDamageAfterAbsorb(damage, (float) player.getArmor(),
                (float) player.getAttributeInstance(EntityAttributes.ARMOR_TOUGHNESS).getValue());
    ...
    private static int sus(ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            ...
            // theres no way this works but it compiles
            r += enchants.getLevel((RegistryEntry<Enchantment>) Enchantments.BLAST_PROTECTION) * 2;
            r += enchants.getLevel((RegistryEntry<Enchantment>) Enchantments.PROTECTION);
        }
        return r;
    }
    ...
    public static double getSwordDamage(ItemStack stack, PlayerEntity player, PlayerEntity target, boolean charged) {
        ...
        if (stack.getEnchantments() != null) {
            if (EnchantmentHelper.getEnchantments(stack)
                    .getLevel((RegistryEntry<Enchantment>) Enchantments.SHARPNESS) > 0) {
                int level = EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.SHARPNESS, stack);
        ...
        DamageSource s = new DamageSource(null, (Entity) mc.player);
        // Reduce by armour
        damage = DamageUtil.getDamageLeft((LivingEntity) target, (float) damage, s, (float) target.getArmor(),
    ...
    public static double bedDamage(LivingEntity player, Box box, Vec3d bed, BlockPos ignore) {
        if (player instanceof PlayerEntity && ((PlayerEntity) player).getAbilities().creativeMode)
            return 0;
        float damage = DamageUtils.bedDamage(player, bed);
        // double modDistance = Math.sqrt(player.squaredDistanceTo(bed));
        // if (modDistance > 10)
        // return 0;

        // double exposure = getExposure(bed, player, box, raycastContext, ignore,
        // true);
        ...
        // // Reduce by armour
        // damage = DamageUtil.getDamageLeft(mc.player, (float) damage,new
        // DamageSource(null), (float) player.getArmor(),
        // (float)
        // player.getAttributeInstance(EntityAttributes.ARMOR_TOUGHNESS).getValue());

        // DamageUtils.bedDamage(player, bed)
        // damage = blastProtReduction(player, damage, explosion);
    ...
    private static double normalProtReduction(Entity player, double damage) {
        return DamageUtils.calculateReductions((float) damage, (LivingEntity) player,
                mc.world.getDamageSources().explosion(null));
        // int protLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(),
        // mc.world.getDamageSources().generic());
        // if (protLevel > 20)
        // protLevel = 20;
    ...
    // Unused and I am not wasting time here
    // private static double blastProtReduction(Entity player, double damage,
    // Explosion explosion) {
    // int protLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(),
    // mc.world.getDamageSources().explosion(explosion));
    // if (protLevel > 20)
    // protLevel = 20;
    ...
    public static double getExposure(Vec3d source, Entity entity, Box box, RaycastContext raycastContext,
            BlockPos ignore, boolean ignoreTerrain) {
            ...
            for (double k = 0; k <= 1; k += d) {
                for (double l = 0; l <= 1; l += e) {
                    for (double m = 0; m <= 1; m += f) {
                        ...
                        ((IVec3d) vec3d).meteor$set(n + g, o, p + h);
                        ((IRaycastContext) raycastContext).meteor$set(vec3d, source, RaycastContext.ShapeType.COLLIDER,
                                RaycastContext.FluidHandling.NONE, entity);

                        if (raycast(raycastContext, ignore, ignoreTerrain).getType() == HitResult.Type.MISS)
                            i++;
    ...
    public static BlockHitResult raycast(RaycastContext context) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            double d = blockHitResult == null ? Double.MAX_VALUE
                    : raycastContext.getStart().squaredDistanceTo(blockHitResult.getPos());
            double e = blockHitResult2 == null ? Double.MAX_VALUE
                    : raycastContext.getStart().squaredDistanceTo(blockHitResult2.getPos());

            return d <= e ? blockHitResult : blockHitResult2;
        }, (raycastContext) -> {
            Vec3d vec3d = raycastContext.getStart().subtract(raycastContext.getEnd());
            return BlockHitResult.createMissed(raycastContext.getEnd(), Direction.getFacing(vec3d.x, vec3d.y, vec3d.z),
