...
public class RaytraceSettings extends BlackOutModule {
    ...
    public boolean placeTrace(BlockPos pos) {
        ...
        switch (placeMode.get()) {
            case SinglePoint -> {
                ((IRaycastContext) raycastContext)
                        .setEnd(new Vec3d(pos.getX() + 0.5, pos.getY() + placeHeight.get(), pos.getZ() + 0.5));
            ...
            case DoublePoint -> {
                ((IRaycastContext) raycastContext)
                        .setEnd(new Vec3d(pos.getX() + 0.5, pos.getY() + placeHeight1.get(), pos.getZ() + 0.5));
                ...
                ((IRaycastContext) raycastContext)
                        .setEnd(new Vec3d(pos.getX() + 0.5, pos.getY() + placeHeight2.get(), pos.getZ() + 0.5));
            ...
            case Sides -> {
                ...
                for (Direction dir : Direction.values()) {
                    ((IRaycastContext) raycastContext)
                            .setEnd(vec.add(dir.getOffsetX() / 2f, dir.getOffsetY() / 2f, dir.getOffsetZ() / 2f));
            ...
            case Exposure -> {
                ...
                for (int x = 0; x <= 2; x += 1) {
                    for (int y = 0; y <= 2; y += 1) {
                        for (int z = 0; z <= 2; z += 1) {
                            ((IRaycastContext) raycastContext)
                                    .setEnd(vec.add(0.1 + x * 0.4, 0.1 + y * 0.4, 0.1 + z * 0.4));
    ...
    public boolean attackTrace(Box box) {
        ...
        switch (attackMode.get()) {
            case SinglePoint -> {
                ((meteordevelopment.meteorclient.mixininterface.IRaycastContext) BODamageUtils.raycastContext)
                        .meteor$set(mc.player.getEyePos(),
                                new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight.get(),
            ...
            case DoublePoint -> {
                ((meteordevelopment.meteorclient.mixininterface.IRaycastContext) BODamageUtils.raycastContext)
                        .meteor$set(mc.player.getEyePos(),
                                new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight1.get(),
                                        (box.minZ + box.maxZ) / 2f),
                                RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);
                ...
                ((meteordevelopment.meteorclient.mixininterface.IRaycastContext) BODamageUtils.raycastContext)
                        .meteor$set(mc.player.getEyePos(),
                                new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight2.get(),
                                        (box.minZ + box.maxZ) / 2f),
                                RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player);
                return BODamageUtils.raycast(BODamageUtils.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case Exposure -> {
                ...
                for (int x = 0; x <= 2; x += 1) {
                    for (int y = 0; y <= 2; y += 1) {
                        for (int z = 0; z <= 2; z += 1) {
                            ((IRaycastContext) raycastContext).setEnd(vec.add(MathHelper.lerp(x / 2f, 0.1, xw - 0.1),
                                    MathHelper.lerp(y / 2f, 0.0, yh - 0.1), MathHelper.lerp(z / 2f, 0.1, zw - 0.1)));
    ...
    private void updateContext() {
        if (raycastContext == null) {
            raycastContext = new RaycastContext(mc.player.getEyePos(), null, RaycastContext.ShapeType.COLLIDER,
