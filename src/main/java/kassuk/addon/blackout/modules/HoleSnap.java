        ...
        if (mc.player != null && mc.world != null) {
            ...
            if (hole != null && !singleBlocked()) {
                ...
                if (mc.player.getX() == hole.middle.x && mc.player.getZ() == hole.middle.z) {
                    ...
                    boostLeft--;
                    ((IVec3d) event.movement).meteor$setXZ(Math.abs(x) < Math.abs(dX) ? x : dX,
                            Math.abs(z) < Math.abs(dZ) ? z : dZ);
                }
            } else {
                this.toggle();
        ...
        for (int x = -range.get(); x <= range.get(); x++) {
            for (int y = -downRange.get(); y < 1; y++) {
                for (int z = -range.get(); z < range.get(); z++) {
                    ...
                    Hole hole = HoleUtils.getHole(pos, singleHoles.get(), doubleHoles.get(), quadHoles.get(),
                            depth.get(), true);
                    ...
                    if (closest == null ||
                            hole.middle.distanceTo(mc.player.getPos()) < closest.middle
