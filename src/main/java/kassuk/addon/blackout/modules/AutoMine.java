...
public class AutoMine extends BlackOutModule {
    ...
    private final Setting<Boolean> resetOnSwitch = sgGeneral.add(new BoolSetting.Builder()
            ...
            .build());
    /*
     * private final Setting<Boolean> mineBeds = sgGeneral.add(new
     * BoolSetting.Builder()
     * .name("Mine Beds")
     ...
     * .build()
     * );
     * private final Setting<Boolean> mineAnchors = sgGeneral.add(new
     * BoolSetting.Builder()
     * .name("Mine Anchors")
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (target != null) {
            if (lastState != null && target.pos.equals(lastPos) && target.manual && manualRemine.get()
                    && !fastRemine.get() && !lastState.isSolid() && OLEPOSSUtils.solid2(target.pos)) {
                started = false;
            }
    ...
    private void explodeUpdate() {
        ...
        if (targetCrystal != null && !isPaused() && mined
                && System.currentTimeMillis() - lastExplode > (1000 / explodeSpeed.get())) {
            if (!SettingUtils.shouldRotate(RotationType.Attacking)
                    || Managers.ROTATION.start(targetCrystal.getBoundingBox(), priority, RotationType.Attacking,
    ...
    private void render(Renderer3D r) {
        ...
        render = MathHelper.clamp(
                getMineTicks(slot, true) == getMineTicks(slot, false) ? render + delta * 2 : render - delta * 2, -2, 2);
        ...
        r.box(getRenderBox(p / 2), getColor(startColor.get(), endColor.get(), p, MathHelper.clamp(render, 0, 1)),
                getColor(lineStartColor.get(), lineEndColor.get(), p, MathHelper.clamp(render, 0, 1)), shapeMode.get(),
                0);
    ...
    private void update() {
        ...
        enemies = mc.world.getPlayers().stream().filter(
                player -> player != mc.player && !Friends.get().isFriend(player) && player.distanceTo(mc.player) < 10)
                .toList();
        ...
        if (target.pos != null && !target.pos.equals(lastPos)) {
            if (started) {
                sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, target.pos,
                        Direction.DOWN, 0));
            }
            started = false;
        }
        ...
        if (!started) {
            boolean rotated = !SettingUtils.startMineRot() || Managers.ROTATION.start(target.pos, priority,
                    RotationType.Mining, Objects.hash(name + "mining"));
            ...
            if (rotated) {
                ...
                sendSequenced(s -> new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK,
                        target.pos, dir == null ? Direction.UP : dir, s));

                SettingUtils.mineSwing(SwingSettings.MiningSwingState.Start);

                mined = false;
                if (mineStartSwing.get())
                    clientSwing(mineHand.get(), Hand.MAIN_HAND);
        ...
        if (!crystalCheck())
            return;
        if (!OLEPOSSUtils.solid2(target.pos))
            return;
    ...
    private void endMine() {
        ...
        if (SettingUtils.shouldRotate(RotationType.Mining)
                && !Managers.ROTATION.start(target.pos, priority, RotationType.Mining, Objects.hash(name + "mining"))) {
            return;
        }
        ...
        sendSequenced(
                s -> new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, target.pos, dir, s));
            ...
            case Cev, TrapCev, SurroundCev -> {
                ...
                if (!EntityUtils.intersectsWithEntity(
                        Box.from(new BlockBox(target.crystalPos))
                                .withMaxY(target.crystalPos.getY() + (SettingUtils.cc() ? 1 : 2)),
    ...
    private boolean placeCrystal() {
        ...
        boolean rotated = !SettingUtils.shouldRotate(RotationType.Interact) || Managers.ROTATION
                .start(target.crystalPos.down(), priority, RotationType.Interact, Objects.hash(name + "placing"));
        ...
        interactBlock(hand == null ? Hand.MAIN_HAND : hand, target.crystalPos.down().toCenterPos(), dir,
    ...
    private Target getCev() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            BlockPos pos = new BlockPos(player.getBlockX(), (int) Math.floor(player.getBoundingBox().maxY) + 1,
                    player.getBlockZ());

            if (!(civ && pos.equals(civPos)) && getBlock(pos) != Blocks.OBSIDIAN) {
                continue;
            }
            if ((civ && pos.equals(civPos)) && !(getBlock(pos) instanceof AirBlock)
            ...
            if (distanceCheck(civ, pos, distance, d)) {
                best = new Target(pos, pos.up(), MineType.Cev,
                        cevPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0), civ, false);
                distance = d;
            }
        }
        return best;
    ...
    Target getTrapCev() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                BlockPos pos = new BlockPos(player.getBlockX(), (int) Math.floor(player.getBoundingBox().maxY),
                        player.getBlockZ()).offset(dir);
                ...
                if (distanceCheck(civ, pos, distance, d)) {
                    best = new Target(pos, pos.up(), MineType.TrapCev,
                            trapCevPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0), civ, false);
                    distance = d;
                }
            }
        }
    ...
    private Target getSurroundCev() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (distanceCheck(civ, pos, distance, d)) {
                    best = new Target(pos, pos.up(), MineType.SurroundCev,
                            surroundCevPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0), civ, false);
                    distance = d;
                }
            }
        }
    ...
    private Target getSurroundMiner() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (distanceCheck(civ, pos, distance, d)) {
                    best = new Target(pos, null, MineType.SurroundMiner,
                            surroundMinerPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0), civ, false);
                    distance = d;
                }
            }
        }
    ...
    private Target getAutoCity() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (distanceCheck(civ, pos, distance, d)) {
                    best = new Target(pos, pos.offset(dir), MineType.AutoCity,
                            autoCityPriority.get().priority + (civ && pos.equals(civPos) ? 0.1 : 0), civ, false);
                    distance = d;
                }
            }
        }
    ...
    private float getTime(BlockPos pos, int slot, boolean speedMod) {
        ...
        if (f == -1.0F) {
            ...
            float i = !state.isToolRequired() || mc.player.getInventory().getStack(slot).isSuitableFor(state) ? 30
                    : 100;
    ...
    private float getSpeed(BlockState state, int slot, boolean speedMod) {
        ...
        ItemStack helmet = mc.player.getInventory().getArmorStack(0);
        boolean hasAquaAffinity = EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.AQUA_AFFINITY,
                helmet) > 0;

        if (waterCheck.get() && mc.player.isSubmergedInWater() && !hasAquaAffinity) {
            f /= 5.0;
        }
    ...
    private int fastestSlot() {
        ...
        for (int i = 0; i < (pickAxeSwitchMode.get() == SwitchMode.Silent ? 9 : 35); i++) {
            if (slot == -1 || (mc.player.getInventory().getStack(i)
                    .getMiningSpeedMultiplier(mc.world.getBlockState(target.pos)) > mc.player.getInventory()
                            .getStack(slot).getMiningSpeedMultiplier(mc.world.getBlockState(target.pos)))) {
                slot = i;
            }
        }
        return slot;
    ...
    private Box getRenderBox(double progress) {
        return new Box(target.pos.getX() + 0.5 - progress, target.pos.getY() + 0.5 - progress,
                target.pos.getZ() + 0.5 - progress, target.pos.getX() + 0.5 + progress,
                target.pos.getY() + 0.5 + progress, target.pos.getZ() + 0.5 + progress);
    }

    private boolean blocked(BlockPos pos) {
        Box box = new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + (SettingUtils.cc() ? 1 : 2),
                pos.getZ() + 1);
