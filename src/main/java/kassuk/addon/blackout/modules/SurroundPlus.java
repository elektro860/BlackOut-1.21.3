...
public class SurroundPlus extends BlackOutModule {
    ...
    private final Setting<Boolean> toggleMove = sgToggle.add(new BoolSetting.Builder()
            ...
            .defaultValue(false)
            .build());
    private final Setting<VerticalToggleMode> toggleVertical = sgToggle
            .add(new EnumSetting.Builder<VerticalToggleMode>()
    ...
    private final Setting<Double> cooldown = sgSpeed.add(new DoubleSetting.Builder()
            .name("Multi Cooldown")
            .description(
                    "Waits x seconds before trying to place at the same position if there is more than 1 missing block.")
            .defaultValue(0.3)
            .min(0)
            .sliderRange(0, 1)
            .build());
    private final Setting<Double> singleCooldown = sgSpeed.add(new DoubleSetting.Builder()
            ...
            .description(
                    "Waits x seconds before trying to place at the same position if there is only 1 missing block.")
            .defaultValue(0.02)
            .min(0)
            .sliderRange(0, 1)
            .build());
    ...
    private void onBlock(BlockUpdateEvent event) {
        if (event.oldState.getBlock() != event.newState.getBlock() && !OLEPOSSUtils.replaceable(event.pos)
                && surroundBlocks.contains(event.pos)) {
    ...
    private void onRender(Render3DEvent event) {
        ...
        surroundBlocks.stream().filter(OLEPOSSUtils::replaceable)
                .forEach(block -> event.renderer.box(block, sideColor.get(), lineColor.get(), shapeMode.get(), 0));
        supportPositions.forEach(block -> event.renderer.box(block, supportSideColor.get(), supportLineColor.get(),
                supportShapeMode.get(), 0));
    ...
    private void updateAttack() {
        ...
        if (SettingUtils.shouldRotate(RotationType.Attacking) && !Managers.ROTATION.start(blocking.getBoundingBox(),
    ...
    private Entity getBlocking() {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (mc.player.distanceTo(entity) > 5)
                continue;
            if (!SettingUtils.inAttackRange(entity.getBoundingBox()))
                continue;

            if (antiCev.get()) {
                for (BlockPos pos : surroundBlocks) {
                    if (entity.getBlockPos().equals(pos.up())) {
                        double dmg = Math.max(10, BODamageUtils.crystal(mc.player, mc.player.getBoundingBox(),
                                entity.getPos(), null, false));
    ...
    private void setBB() {
        if (!centered && center.get() && mc.player.isOnGround() && (!phaseCenter.get()
                || !OLEPOSSUtils.inside(mc.player, mc.player.getBoundingBox().shrink(0.01, 0.01, 0.01)))) {
            ...
            if (dist < 0.2873) {
                sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(targetX, mc.player.getY(), targetZ,
            ...
            for (int i = 0; i < Math.ceil(dist / 0.2873); i++) {
                ...
                sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(x, mc.player.getY(), z,
                        Managers.ON_GROUND.isOnGround(), mc.player.horizontalCollision));
            }

            mc.player.setPos(targetX, mc.player.getY(), targetZ);
            mc.player.setBoundingBox(new Box(targetX - 0.3, mc.player.getY(), targetZ - 0.3, targetX + 0.3,
                    mc.player.getY() + (mc.player.getBoundingBox().maxY - mc.player.getBoundingBox().minY),
                    targetZ + 0.3));
    ...
    private void placeBlocks() {
        ...
        valids.stream().filter(pos -> !EntityUtils.intersectsWithEntity(Box.from(new BlockBox(pos)), this::validEntity))
    ...
    private void place(BlockPos pos) {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && !Managers.ROTATION.start(data.pos(), priority,
                RotationType.BlockPlace, Objects.hash(name + "placing"))) {
            return;
        }
    ...
    private void setBlock(BlockPos pos) {
        ...
        mc.world.playSound(pos.getX(), pos.getY(), pos.getZ(), SoundEvents.BLOCK_STONE_PLACE, SoundCategory.BLOCKS, 1,
                1, false);
    }
    ...
    private boolean valid(ItemStack stack) {
        return stack.getItem() instanceof BlockItem block
                && (support ? supportBlocks : blocks).get().contains(block.getBlock());
    }
    ...
    private void addSupport(BlockPos pos) {
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (EntityUtils.intersectsWithEntity(Box.from(new BlockBox(pos.offset(dir))),
                    entity -> entity instanceof PlayerEntity && !entity.isSpectator()))
                continue;
            if (!SettingUtils.getPlaceData(pos.offset(dir)).valid())
                continue;
    ...
    private void updateInsideBlocks() {
        ...
        if (extend.get()) {
            mc.world.getPlayers().stream().filter(player -> mc.player.distanceTo(player) < 5 && player != mc.player)
                    .sorted(Comparator.comparingDouble(player -> mc.player.distanceTo(player))).forEach(player -> {
    ...
    private void addBlocks(BlockPos pos, int[] size) {
        for (int x = size[0]; x <= size[1]; x++) {
            for (int z = size[2]; z <= size[3]; z++) {
                ...
                if (mc.world.getBlockState(p).getBlock().getBlastResistance() > 600 && !p.equals(currentPos))
                    continue;
