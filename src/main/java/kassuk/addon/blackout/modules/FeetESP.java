...
public class FeetESP extends BlackOutModule {
    ...
    private void onRender(Render3DEvent event) {
        ...
        mc.world.getPlayers().forEach(player -> {
            ...
            if (!friend.get() && Friends.get().isFriend(player))
                return;
            if (!other.get() && player != mc.player && !Friends.get().isFriend(player))
                return;
            if (!self.get() && mc.player == player)
                return;
    ...
    private void render(Render3DEvent event, Vec3d vec) {
        event.renderer.sideHorizontal(vec.x - 0.3, vec.y, vec.z - 0.3, vec.x + 0.3, vec.z + 0.3, sideColor.get(),
