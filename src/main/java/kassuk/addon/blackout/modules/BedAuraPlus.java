...
public class BedAuraPlus extends BlackOutModule {
    public BedAuraPlus() {
        super(BlackOut.BLACKOUT, "Bed Aura+",
                "Automatically places and breaks beds to cause damage to your opponents but better.");
    }
    ...
    private final Setting<RotationMode> rotMode = sgGeneral.add(new EnumSetting.Builder<RotationMode>()
            .name("Rotation Mode")
            .description(
                    "Packet- Sends 1 rotation packet for each bed. Manager- Modifies movement packets to set rotation.")
    ...
    private void onTickPre(TickEvent.Post event) {
        ...
        blocks = getBlocks(mc.player.getEyePos(),
                Math.max(SettingUtils.getPlaceRange(), SettingUtils.getPlaceWallsRange()));
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (renderPos != null && pauseCheck()) {
            ...
            if (renderDir != null) {
                event.renderer.box(bedBox(renderPos.offset(renderDir)), fColor.get(), fLineColor.get(), shapeMode.get(),
                        0);
            }
        }
    }
    ...
    private void calculate(int index) {
        ...
        for (int i = lastIndex; i < index; i++) {
            ...
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (!OLEPOSSUtils.replaceable(pos.offset(dir))
                        && !(mc.world.getBlockState(pos.offset(dir)).getBlock() instanceof BedBlock))
                    continue;

                if (!SettingUtils.inPlaceRange(data.pos()))
                    continue;

                if (!fiveB.get() && EntityUtils.intersectsWithEntity(new Box(pos.offset(dir)),
        ...
        for (int x = -i; x <= i; x++) {
            for (int y = -i; y <= i; y++) {
                for (int z = -i; z <= i; z++) {
                    ...
                    if (fiveB.get() && (mc.world.getBlockState(pos.down()).getBlock() == Blocks.AIR
                            || mc.world.getBlockState(pos.down()).hasBlockEntity()))
                        continue;
    ...
    private void update() {
        if (placePos == null || placeData == null || !placeData.valid() || bedDir == null)
            return;
        ...
        if (logicMode.get() == LogicMode.PlaceBreak) {
            ...
            if (OLEPOSSUtils.replaceable(placePos) && OLEPOSSUtils.replaceable(placePos.offset(bedDir))
                    && placeUpdate()) {
                removeBed2(placePos);
    ...
    private List<BlockPos> interactUpdate() {
        if (doubleInteract.get()) {
            if (SettingUtils.shouldRotate(RotationType.Interact) && !Managers.ROTATION.start(placePos, priority,
                    RotationType.Interact, Objects.hash(name + "explode"))) {
                return null;
            }
        ...
        if (SettingUtils.shouldRotate(RotationType.Interact) && !Managers.ROTATION.start(interactPos, priority,
                RotationType.Interact, Objects.hash(name + "explode"))) {
            return null;
        }
        ...
        if (interactSwing.get())
            clientSwing(interactHand.get(), Hand.MAIN_HAND);
    ...
    private BlockPos getInteractPos() {
        if (isBed(placePos.offset(bedDir)) && SettingUtils.inPlaceRange(placePos.offset(bedDir))
                && SettingUtils.getPlaceOnDirection(placePos.offset(bedDir)) != null) {
            return placePos.offset(bedDir);
        }
        if (isBed(placePos) && SettingUtils.inPlaceRange(placePos)
                && SettingUtils.getPlaceOnDirection(placePos) != null) {
            return placePos;
        }
        return null;
    }
    ...
    private boolean placeUpdate() {
        Hand hand = Managers.HOLDING.getStack().getItem() instanceof BedItem ? Hand.MAIN_HAND
                : mc.player.getOffHandStack().getItem() instanceof BedItem ? Hand.OFF_HAND : null;
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && !Managers.ROTATION.start(placeData.pos(), priority,
        ...
        if (rotMode.get() == RotationMode.Packet) {
            sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(bedDir.getOpposite().asRotation(),
            ...
            if (Math.abs(
                    RotationUtils.yawAngle(Managers.ROTATION.lastDir[0], bedDir.getOpposite().asRotation())) > 45) {
                return false;
            }
        }
    ...
    private double getDmg(BlockPos pos) {
        double highest = -1;
        for (PlayerEntity target : targets) {
            highest = Math.max(highest, BODamageUtils.bedDamage(target, target.getBoundingBox(),
                    new Vec3d(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5), null));
        }
        return highest;
    }
    ...
    private void damageCalc(BlockPos pos) {
        ...
        // Self
        self = BODamageUtils.bedDamage(mc.player, mc.player.getBoundingBox(),
                new Vec3d(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5), null);
        ...
        for (PlayerEntity friend : friends) {
            ...
            highest = Math.max(highest, BODamageUtils.bedDamage(friend, friend.getBoundingBox(),
                    new Vec3d(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5), null));
    ...
    private PlaceData getData(BlockPos pos, Direction dir) {
        if (fiveB.get()) {
            return SettingUtils.getPlaceDataAND(pos.offset(dir), direction -> direction == Direction.DOWN,
                    pos1 -> !(mc.world.getBlockState(pos1).getBlock() instanceof BedBlock));
        } else {
            return SettingUtils.getPlaceDataAND(pos.offset(dir), direction -> direction != dir,
                    pos1 -> !(mc.world.getBlockState(pos1).getBlock() instanceof BedBlock));
        }
    }
